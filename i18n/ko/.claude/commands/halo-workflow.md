...
- 실제 실행 환경에서 수행 (Mock 금지)
- 실제 서버/서비스 기동
- 실제 브라우저/클라이언트 사용 (웹 프로젝트의 경우)
- 실제 외부 API 연동 (또는 실제 프록시 경유)
...
E2E 실행에 필요한 환경 구성을 이 Phase에서 함께 준비한다.
(서버 스크립트, 환경 변수, 테스트 설정 파일 등)
...
```
검증 항목:
- E2E 테스트에서 fetch/HTTP mock 사용 여부 → 사용 시 FAIL
- E2E 테스트에서 실제 서버 기동 여부 → 미기동 시 FAIL
- Smoke Test 통과 여부

위 조건 미충족 시 → P6으로 LOOPBACK (사유: "E2E 미달")
```
